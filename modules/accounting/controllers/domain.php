...
class domain extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Admin.Accounting.Domain',"dashboard");
	...
	public function index()
	{
		...
		if ($this->form_validation->run()) {
	        ...
	        $this->db->where('option', 'domain');
        	$this->db->update('setting', $data_escompte);
	        Template::set_message($this->lang->line('domain_price_updated'), 'success');
		...
		if($query->result())
		{
			...
			Template::set('title', $this->lang->line('domain_price'));
			Template::render();
		}
	}
